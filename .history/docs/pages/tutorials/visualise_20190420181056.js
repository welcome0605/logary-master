  ...
  return (
    <DocPage name="js-quickstart" title="Tutorials" faIcon={faFileChartLine} colour="primary" readingMinutes={3} toc={toc}>
      ...
      <DocSection {...toc[2]}>
        ...
        <p></p>
        <p>Things that end-users of the library are not likely to configure should go in <span className="_code">Logary.Internals</span> . Examples include <span className="_code"> Logary.Internals.Globals </span> and Logary.Internals.RuntimeInfo (which is configured with the config API instead).</p>
      </DocSection>
    </DocPage>
  )
}