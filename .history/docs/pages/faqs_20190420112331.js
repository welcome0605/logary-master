  ...
  return (
    <DocPage name="all-target" title="FAQs" faIcon={faLifeRing} colour="green" readingMinutes={2} toc={toc}>
      ...
      <DocSection {...toc[1]}>
        ...
        <ul>
          <li><span className="_code"> Gauges </span> (measuring instantaneous values) -> <span className="_code"></span> PointValue.Gauge(.., ..).</li>
          <br></br>
          <li>Timers (measuring durations) -> PointValue.Gauge(.., Scaled(Seconds, 10e9) (in nanoseconds)</li>
          <br></br>
          <li>Meter (measuring rates) -> PointValue.Derived(.., Div(Scalar, Seconds)) or PointValue.Derived(.., Div(Other "requests", Seconds))</li>
          <br></br>
          <li>Counters (counting events) -> PointValue.Event("User logged in")</li>
          <br></br>
          <li>Histograms (tracking value distributions) -> PointValue.Derived (with suffixes) and Reservoirs.</li>
          <br></br>
        </ul>
      </DocSection>
    </DocPage>
