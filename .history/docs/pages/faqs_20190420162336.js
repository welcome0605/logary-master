    ...
    <DocPage name="all-target" title="FAQs" faIcon={faLifeRing} colour="green" readingMinutes={2} toc={toc}>
      ...
      <DocSection {...toc[3]}>
        ...
        <h3>What's logVerboseWithAck, logWithAck and how does it differ from logSimple?</h3>
          ...
          <p>Because running out of memory generally is unwanted, each target has a RingBuffer that the messages are put into when you use the Logger. Unless all targets' RingBuffer accept the Message, the call to log doesn't complete. This is similar to how other logging frameworks work.</p>
      </DocSection>
    </DocPage>
  )
}
