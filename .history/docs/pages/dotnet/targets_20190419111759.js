  ...
  return (
    <DocPage name="all-target" title="Target / Sinks" faIcon={faPuzzlePiece} colour="pink" readingMinutes={2} toc={toc}>
      ...
      <DocSection {...toc[1]}>
        ...
        <h3>Performance</h3>
          <p>The <span className="_code"> File target </span> is a performance-optimised target. Logging always happens on a separate thread from the caller, so we try to reach a balance between throughput and latency on ACKs.</p>
      </DocSection>
    </DocPage>
  )
}