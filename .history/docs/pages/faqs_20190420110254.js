  ...
  return (
    <DocPage name="all-target" title="FAQs" faIcon={faLifeRing} colour="green" readingMinutes={2} toc={toc}>
      ...
      <DocSection {...toc[1]}>
        ...
        <p>Starting with an <span className="_code"></span> Event; this is the main value when you're logging (in fact, it's Logary.PointValue.Event(template:string) that you're using.) An event is like a Gauge at a particular instant on the global timeline with a value of 1 (one).</p> 
        <p>Why Logary instead of one of the classic logging frameworks?</p> 
        <p>Why Logary instead of one of the classic logging frameworks?</p> 
        <p>Why Logary instead of one of the classic logging frameworks?</p> 
        <p>Why Logary instead of one of the classic logging frameworks?</p> 
        <ul>
